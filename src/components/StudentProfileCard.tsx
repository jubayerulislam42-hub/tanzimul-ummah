...
import type { StudentRecord } from "@/lib/auth";

export default function StudentProfileCard({ record }: { record: StudentRecord }) {
  return (
    <div className="rounded-2xl border border-primary/10 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 space-y-2 text-sm">
        ...
        {record.guardian_phone && (
          <div className="flex items-center gap-2 text-charcoal/70">
            ...
            <span>{record.guardian_phone}</span>
          </div>
        )}
      </div>
      ...
      <div className="mt-3 border-t border-primary/5 pt-3">
        ...
        >
          {record.status === "active" ? "সক্রিয় ছাত্র" : "নিষ্ক্রিয়"}
        </span>
      </div>
    </div>
  );
